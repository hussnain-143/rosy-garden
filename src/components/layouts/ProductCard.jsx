...
import { FaShoppingCart, FaBan , FaCheckCircle } from "react-icons/fa";
import {cartContext} from "../../context/CartContext"
import { useContext  } from "react";

const ProductCard = ({ id, image, title, desc, price, inStock}) => {
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-md group">
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-between p-4 text-white">
          ...
          <div className="flex justify-between items-center">
            ...
            {inStock ? (
              <button className="flex items-center gap-1 px-2 py-1 rounded cursor-pointer text-xs font-medium bg-green-custom" onClick={ () => dispatch({ type: 'ADD_ITEM', payload: { id, title, price, image, qty: 1 } }) }>
